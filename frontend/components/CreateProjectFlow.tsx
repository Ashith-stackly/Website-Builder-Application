...
const CreateProjectModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-2 sm:p-6 overflow-hidden">
      ...
      {/* Modal Card */}
      <div className="relative w-full max-w-[95vw] sm:max-w-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl sm:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[95vh] sm:max-h-[90vh] text-slate-900 dark:text-slate-100">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full border-2 rounded-xl sm:rounded-2xl px-4 py-3 sm:px-6 sm:py-4 text-base sm:text-lg outline-none transition-all font-bold text-[#06224C] dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 ${
                    error
                      ? "border-red-400 dark:border-red-500 bg-red-50 dark:bg-red-950/60"
                      : "border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-950 focus:border-blue-500 dark:focus:border-blue-400 focus:bg-white dark:focus:bg-slate-900"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {categories.map((cat) => (
                  <button
                    ...
                    className={`p-3 sm:p-5 rounded-xl sm:rounded-2xl border-2 text-left transition-all cursor-pointer ${
              ...
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
                {templateStyles.map((style) => (
                  <button
                    ...
                    className={`group cursor-pointer space-y-2 rounded-xl sm:rounded-2xl border-2 p-3 text-left transition-all ${
        ...
        <div className="p-4 sm:p-6 bg-gray-50 dark:bg-slate-950 border-t border-gray-100 dark:border-slate-800 flex flex-wrap items-center gap-2 shrink-0">
          {step > 1 && (
            <button
              onClick={handleBack}
              className="px-4 py-2 sm:px-8 sm:py-3 font-black text-[10px] sm:text-xs uppercase tracking-widest text-gray-400 dark:text-slate-500 hover:text-[#06224C] dark:hover:text-slate-200 transition-colors cursor-pointer"
            >
              Back
            </button>
          )}
          ...
          {step < 4 ? (
            ...
            <button
              ...
              className="ml-auto bg-green-600 dark:bg-green-500 text-white px-6 py-2 sm:px-10 sm:py-3 rounded-lg sm:rounded-xl font-black text-[10px] sm:text-xs uppercase tracking-widest hover:bg-green-700 dark:hover:bg-green-600 transition-all shadow-lg disabled:opacity-50 whitespace-nowrap cursor-pointer disabled:cursor-not-allowed"
            >
