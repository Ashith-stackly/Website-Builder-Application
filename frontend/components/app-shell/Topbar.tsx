  ...
  return (
    ...
    >
      <button
        ...
        className="grid h-9 cursor-pointer w-9 place-items-center rounded-lg lg:hidden"
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            className="relative grid cursor-pointer h-7 w-7 place-items-center rounded-lg"
            aria-label={o.label}
            aria-checked={active}
            role="radio"
  ...
  return (
    <div ref={ref} className="relative">
      <motion.button
        whileTap={{ scale: 0.92 }}
        onClick={() => setOpen((v) => !v)}
        className="relative grid cursor-pointer h-9 w-9 place-items-center rounded-xl border"
        ...
        {open && (
          ...
          >
            <div className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: "var(--border)" }}>
              ...
              <button className="text-xs cursor-pointer font-semibold" style={{ color: "var(--accent)" }}>Mark all read</button>
            </div>
            <motion.ul variants={staggerContainer} initial="hidden" animate="visible" className="max-h-80 overflow-y-auto p-1.5">
              {SAMPLE_NOTIFICATIONS.map((n) => (
                <motion.li key={n.id} variants={staggerChild}>
                  <button className="flex w-full cursor-pointer items-start gap-3 rounded-xl p-2.5 text-left transition-colors hover:bg-[color:var(--surface-2)]">
        ...
        {open && (
          ...
          >
            <div className="flex items-center gap-3 border-b px-4 py-3" style={{ borderColor: "var(--border)" }}>
              <span className="grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br from-[#4f6bed] to-[#8b5cf6] text-sm font-bold text-white overflow-hidden">
                {user?.avatar ? <img src={user.avatar} alt="Avatar" className="h-full w-full object-cover" /> : initials}
              </span>
...
function MenuLink({
  ...
  return (
    <button
      onClick={onClick}
      className="flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-[13px] font-medium transition-colors hover:bg-[color:var(--surface-2)]"
