    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 overflow-hidden rounded-xl border border-[#0B1D40] sm:grid-cols-3">
          {VARIANTS.map(({ value, label }) => (
            <button key={value} type="button" onClick={() => setProp("variant", value)} className={`cursor-pointer border-b border-r border-[#0B1D40]/10 px-2 py-2 text-[10px] font-bold transition last:border-r-0 ${data.variant === value ? "bg-[#0B1D40] text-white" : "text-[#0B1D40] hover:bg-black/5"}`}>
              {label}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          ...
          <select value={data.selection} onChange={(event) => setProp("selection", event.target.value as ProductCollectionData["selection"])} className="cursor-pointer w-full rounded-lg border border-[#0B1D40] bg-white px-3 py-2 text-[12px] font-semibold text-[#0B1D40] outline-none focus:ring-2 focus:ring-blue-100">
        ...
        <label className="block">
          ...
          <select value={data.sort} onChange={(event) => setProp("sort", event.target.value as ProductCollectionData["sort"])} className="cursor-pointer w-full rounded-lg border border-[#0B1D40] bg-white px-3 py-2 text-[12px] font-semibold text-[#0B1D40] outline-none focus:ring-2 focus:ring-blue-100">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <label className="block">
          ...
          <select value={data.limit} onChange={(event) => setProp("limit", Number(event.target.value))} className="cursor-pointer w-full rounded-lg border border-[#0B1D40] bg-white px-3 py-2.5 text-[12px] font-semibold text-[#0B1D40] outline-none focus:ring-2 focus:ring-blue-100">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 overflow-hidden rounded-xl border border-[#0B1D40]">
          {([2, 3, 4] as const).map((columns) => (
            <button key={columns} type="button" onClick={() => setProp("columns", columns)} className={`cursor-pointer py-2 text-xs font-bold transition ${data.columns === columns ? "bg-[#0B1D40] text-white" : "text-[#0B1D40] hover:bg-black/5"}`}>
              {columns}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        {([
          ...
          <button key={key} type="button" onClick={() => setProp(key, !data[key])} className={`cursor-pointer rounded-lg border px-2 py-2 text-[11px] font-bold transition ${data[key] ? "border-[#0B1D40] bg-[#0B1D40] text-white" : "border-[#dbe3ef] bg-white text-[#566583] hover:border-blue-300"}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3 border-t border-[#dbe3ef] pt-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <button type="button" onClick={addProduct} disabled={data.products.length >= 12} className="cursor-pointer inline-flex items-center gap-1 rounded-lg bg-[#0B1D40] px-2.5 py-1.5 text-[11px] font-bold text-white transition hover:bg-[#152B52] disabled:cursor-not-allowed disabled:opacity-50">
        ...
        {data.products.map((product, index) => (
          <div key={product.id || `${product.name}-${index}`} className="space-y-3 rounded-xl border border-[#dbe3ef] bg-white/70 p-3">
            <div className="flex items-start gap-2">
              ...
              <button type="button" onClick={() => removeProduct(index)} disabled={data.products.length <= 1} aria-label={`Remove ${product.name || "product"}`} className="cursor-pointer mt-7 rounded p-1 text-[#566583] transition hover:bg-red-50 hover:text-red-500 disabled:cursor-not-allowed disabled:opacity-40"><X className="h-4 w-4" /></button>
            </div>
            <div className="grid grid-cols-2 gap-3">
