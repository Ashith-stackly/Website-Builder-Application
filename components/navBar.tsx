    ...
    >
      <nav ref={navRef} className="mx-auto flex w-full max-w-7xl flex-wrap items-center justify-between gap-2 overflow-visible md:gap-4">
        <div className="flex min-w-0 items-center gap-1 md:gap-8">
          <button
            type="button"
            className="inline-flex h-9 w-9 items-center justify-center rounded-md p-1 text-white transition hover:bg-white/10 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#06224C] xl:hidden"
      ...
      {mobileOpen && (
        ...
        >
          <div className="flex flex-col">
                  ...
                  {navCategories.map(({ title, icon: Icon, items }) => (
                    <div key={`mobile-${title}`}>
                      ...
                      {mobileCategory === title && (
                        <div className="border-b border-white/5 bg-[#051a3d] py-2">
                          {items.map((item) => (
                            <Link key={`mobile-${title}-${item}`} href={item === "Video Block" ? "/video-block" : "/landing#categories"} onClick={(event) => { if (item !== "Video Block") scrollLandingSection(event, "categories", true); else setMobileOpen(false); }} className="block px-14 py-3 text-[10px] text-gray-300 focus-visible:outline-none focus-visible:bg-white/10 focus-visible:text-white">
                              {item}
                            </Link>
                          ))}
                        </div>
            ...
            <motion.div variants={mobileItemVariants} whileTap={{ scale: 0.97 }} transition={{ type: "spring", stiffness: 500, damping: 28 }}><Link href="/video-block" onClick={() => setMobileOpen(false)} className="block px-6 py-4 focus-visible:outline-none focus-visible:bg-white/10">Video Block</Link></motion.div>
