...
const WHATSNEW = {
  title: "Source audit reconciled with the unified backend",
  body:
    "This pass replaces the legacy split-backend narrative and maps the dashboard to the active Express routes and Next.js clients. " +
    "It also calls out the main contract breaks: templates, hosted publishing, domains, storefront checkout, and local-only settings."
};
...
const PROJECT = {
  ...
  stack: [
    ...
    ["Builder", "@dnd-kit + 27+ block types"],
...
const MODULES = [
  ...
  },
  {
    ...
    blurb: "The active builder has a strong flow-layout editing core: nested drag-and-drop, block editing, JSON import/export, autosave, and undo/redo. The separate freeform canvas exists but is not rendered by the active editor.",
    tasks: [
      ...
      { t: "Implement drag-and-drop system", s: "done",
        p: ["frontend/components/builder/BuilderLayout.tsx", "frontend/components/builder/SortableItem.tsx"],
        note: "The active editor uses dnd-kit for block insertion and nested sortable reordering." },
      ...
      { t: "Implement component positioning (drag/move)", s: "partial",
        p: ["frontend/components/builder/FreeformCanvas.tsx", "frontend/components/builder/Canvas.tsx"],
        note: "Freeform positioning components exist, but the active Canvas does not render FreeformCanvas; general blocks remain reorderable rather than freely movable." },
      ...
      { t: "Save page structure as JSON", s: "done",
        ...
        note: "Projects can be exported as JSON and persisted to the backend through autosave." },
      ...
      { t: "Implement undo/redo functionality", s: "done",
        ...
        note: "The builder exposes a capped history stack and keyboard shortcuts." },
    ],
  },
  {
  ...
  },
  {
    ...
    blurb: "Product, order, cart, and checkout backend services exist. Admin product and order screens exist too, but the catalog list contract is mismatched and the public storefront still uses hard-coded products, local cart state, and a demo payment helper.",
    tasks: [
      ...
      { t: "Design product management UI", s: "partial",
        p: ["frontend/app/dashboard/products/page.tsx", "frontend/lib/ecommerceApi.ts"],
        note: "The management screen is built, but listProducts expects an array while the backend returns { products, pagination }." },
      { t: "Add products to pages", s: "pending",
        p: [], note: "No product/catalog/cart block is registered in the builder." },
      { t: "Implement cart system (add/remove/update)", s: "partial",
        p: ["backend/src/services/cartService.js", "backend/src/routes/cartRoutes.js"],
        note: "Cart services and routes exist, but no storefront UI calls them and the cart router lacks the authentication middleware its controller expects." },
      { t: "Build checkout flow", s: "partial",
        p: ["backend/src/services/checkoutService.js", "backend/src/routes/checkoutRoutes.js"],
        note: "Order creation and verification services exist; the public storefront does not call checkout APIs." },
      { t: "Integrate payment gateway", s: "partial",
        p: ["backend/src/services/checkoutService.js", "frontend/lib/razorpayClient.ts"],
        note: "Backend Razorpay verification is implemented, but public checkout uses a separate demo/local helper rather than this flow." },
  ...
  },
  {
    ...
    tasks: [
      ...
      { t: "Integrate Google Analytics (optional)", s: "pending",
        p: [], note: "No Google Analytics integration was found." },
    ],
  },
  {
